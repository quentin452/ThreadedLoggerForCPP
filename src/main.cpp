...
void testLoggingSpeed() {
  ...
  for (int i = 0; i < numIterations; ++i) {
    CreateGlobalsLoggerInstanceExample::LoggerInstance
        .logMessageAsync(LogLevel::INFO, __FILE__, __LINE__, "logger test");
  }
  auto endLogAsync = std::chrono::high_resolution_clock::now();
  auto durationLogAsync =
      std::chrono::duration_cast<std::chrono::milliseconds>(endLogAsync - startLogAsync);
  ...
  auto durationStdCout =
      std::chrono::duration_cast<std::chrono::milliseconds>(endStdCout - startStdCout);
  std::string durationStdCoutStr =
      "Time taken by std::cout: " + std::to_string(durationStdCout.count()) + " milliseconds\n";
  CreateGlobalsLoggerInstanceExample::LoggerInstance
      .logMessageAsync(LogLevel::INFO, __FILE__, __LINE__, durationStdCoutStr);
  std::string durationLogAsyncStr =
      "Time taken by logMessageAsync: " + std::to_string(durationLogAsync.count()) +
      " milliseconds\n";
  CreateGlobalsLoggerInstanceExample::LoggerInstance
      .logMessageAsync(LogLevel::INFO, __FILE__, __LINE__, durationLogAsyncStr);
}
void testLoggingGetThreadSpeed() {
  ...
  for (int i = 0; i < numIterations; ++i) {
    LoggerThread::GetLoggerThread()
        .logMessageAsync(LogLevel::INFO, __FILE__, __LINE__, "Test message with GetLoggerThread");
  }
  ...
  std::string test1 =
      "Time taken with calling GetLoggerThread each time: " + std::to_string(duration1.count()) +
      " milliseconds\n";
  CreateGlobalsLoggerInstanceExample::LoggerInstance
      .logMessageAsync(LogLevel::INFO, __FILE__, __LINE__, test1);
  std::string test2 = "Time taken with storing the result of GetLoggerThread: " +
                      std::to_string(duration2.count()) + " milliseconds\n";
  CreateGlobalsLoggerInstanceExample::LoggerInstance
      .logMessageAsync(LogLevel::INFO, __FILE__, __LINE__, test2);
}

void InitThreadedLoggerForCPP(
    std::string &ProjectDirectory, std::string &LogFileName, std::string &GameSaveFolder) {
#pragma warning(push)
